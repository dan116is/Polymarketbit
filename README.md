...
| שומר־ענן (באתר החי) | ‏Durable Object עם אלרם שרץ 24/7 בענן: מתעד **כל סיבוב** (מחירי OKX + התוצאה הרשמית של Polymarket מ-Gamma‏ `closed=true`), מעריך איתותים בחלון הפעולה ושולח Web Push לטלפון גם כשהאפליקציה סגורה. רשומות שנכתבו לפי פרוקסי מחיר משודרגות אוטומטית לתוצאה הרשמית ברגע שהיא מתפרסמת (כולל תיקון PnL והתראת תיקון אם התוצאה התהפכה). ההיסטוריה נשמרת ב-KV ונמשכת לאפליקציה דרך `/api/history`. הערה: בינאנס חוסמת יציאות Cloudflare‏ (403/451) — לכן מקור המחיר בענן הוא OKX (נרות שנייה) |
...
- **M6 בשלב צל (אושר במפורש)** — עקומת ה-EV של בוט נמדדה חיובית ב-latency
  אפס (‎+2.66¢) ושלילית מ-2 שניות; לכן רץ **בוט צל**: חותם הזמנות אמיתיות
  ‏offline‏ (EIP-712) ומודד את זמן הבנייה בלי לשלוח כלום. ‏LIVE executor קיים
  אבל מסרב לירות עד: דריכה מפורשת + GATE1B/GATE2B ירוקים + מפתח ארנק —
  אין override. ההחלטה נדרשת מחדש אחרי ≥100 מדידות צל
  (`scripts/gate_bot_check.py`).
